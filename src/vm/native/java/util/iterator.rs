...
pub(crate) fn iter_has_next(vm: &mut Vm, args: &[JValue]) -> R {
    ...
    let len = match &k {
        ...
        | IterKind::MapValues { map, .. } => match payload(vm, JValue::Obj(*map)) {
...
pub(crate) fn iter_next(vm: &mut Vm, args: &[JValue]) -> R {
    ...
    let item = match &k {
        IterKind::List { list, idx } => match payload(vm, JValue::Obj(*list)) {
            Some(Native::List(items)) => items.get(*idx).copied().ok_or_else(|| no_such_elem(vm)),
            #[cfg(feature = "jsoup")]
            Some(Native::SFilterList(items)) => {
                items.get(*idx).copied().ok_or_else(|| no_such_elem(vm))
            }
            _ => Err(npe(vm)),
        },
        IterKind::Set { set, idx } => match payload(vm, JValue::Obj(*set)) {
            Some(Native::Set(items)) => items.get(*idx).copied().ok_or_else(|| no_such_elem(vm)),
            _ => Err(npe(vm)),
        },
        IterKind::MapEntries { map, idx } => alloc(
            vm,
